...
def run_get(num_threads):
    ...
    try:
        ...
        logger.warning(f"GET   | {SEC_LIST_FILE} not found.")
    except KeyError:
        logger.warning(f"GET   | 'Symbol' column not found in {SEC_LIST_FILE}.")
    ...
    try:
        ...
        logger.warning(f"GET   | {IND_LIST_FILE} not found.")
    except KeyError:
        logger.warning(f"GET   | 'Symbol' column not found in {IND_LIST_FILE}.")
...
def run_save():
    ...
    for filename in csv_files:
        ...
        if len(rows) < 2:
            ...
            logger.debug(f"Skipping {filename}: insufficient data ( < 2 rows).")
            continue
        try:
            ...
            for day in [latest_day, previous_day]:
                for key, value in day.items():
                    try:
                        if value and value.replace(".", "", 1).isdigit(): day[key] = float(value)
            ...
            latest_day["Change"] = ((adj_close_latest - adj_close_previous) / adj_close_previous * 100) if adj_close_previous != 0 else 0
    ...
    for filename in csv_files:
                ...
                for row in reader:
                    try:
                        if all(k in row and row[k] for k in ["Datetime", "Close", "Adj Close"]):
                            json_data.append({"Date": row["Datetime"].split(" ")[0], "Close": float(row["Close"]), "Adj Close": float(row["Adj Close"])})
        ...
        if len(json_data) >= 14:
            ...
            skipped_ind_count += 1
            logger.debug(f"Skipped individual JSON for {filename} (less than 14 records)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download and process stock data.", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--get", action="store_true", help="Only download stock data as CSVs.")
    parser.add_argument("--save", action="store_true", help="Only convert existing CSVs to JSON.")
    parser.add_argument("-n", "--num-threads", type=int, default=12, help="Number of parallel threads for downloading.")
    ...
    coloredlogs.install(
        ...
        logger=logger,
        fmt=fmt,
        level_styles={'info': {'color': 'cyan'}, 'warning': {'color': 'yellow'}, 'error': {'color': 'red'}, 'critical': {'color': 'red', 'bold': True}}
    )
